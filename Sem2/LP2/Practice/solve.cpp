...
public:
    ...
    bool safe2(vector<vector<int>> &board, vector<bool> &rows, vector<bool> &left, vector<bool> &right, int col)
    {
        if (col >= n)
        {
            return true;
        ...
        for (int row = 0; row < n; row++)
        {
            if (isSafe2(row, col, rows, left, right))
            {
                ...
                if (safe2(board, rows, left, right, col + 1))
                    return true;
...
int main()
{
    ...
    while (1)
    {
        ...
        case 2:
        {
            ...
            bool ans = obj.safe2(board, rows, left, right, 0);
